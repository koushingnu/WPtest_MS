...
class AIAP_Lite_Box {
    ...
    function __construct() {
        ...
        add_action('wp_ajax_aiap_run_start',     array($this, 'ajax_run_start'));
        add_action('wp_ajax_aiap_check_progress', array($this, 'ajax_check_progress'));
        add_action('wp_ajax_aiap_do_job',        array($this, 'ajax_do_job'));
    ...
    function register() {
        ...
        add_settings_field('sub_topics_manager', '中項目の管理', function() {
            // 新規追加の処理
            if (isset($_POST['add_topic']) && check_admin_referer('aiap_topic_action')) {
                $o = get_option(self::OPT_KEY, array());
            ...
            // 削除の処理
            if (isset($_POST['delete_topic']) && check_admin_referer('aiap_topic_action')) {
                $o = get_option(self::OPT_KEY, array());
                ...
                <form method="post" style="margin-bottom: 20px; padding: 15px; background: #f8f8f8; border-radius: 5px;">
                    ...
                    <div style="display: flex; gap: 10px; margin-bottom: 10px;">
                        <input type="text" name="new_topic_title" class="regular-text" 
                               placeholder="項目タイトル（例：治療法と効果）" style="flex: 2;">
                        ...
                        <div class="topic-item" style="margin-bottom: 10px; padding: 10px; border: 1px solid #ddd; border-radius: 4px;">
                            <div style="display: flex; gap: 10px; align-items: center;">
                                ...
                                <form method="post" style="margin: 0;">
                                    ...
                                    <input type="submit" name="delete_topic" class="button button-link-delete" value="削除" 
                                           onclick="return confirm('この項目を削除してもよろしいですか？');">
                                </form>
                            </div>
                        </div>
                    <?php endforeach; ?>
    ...
    function admin_scripts($hook) {
        ...
        wp_localize_script('aiap-admin', 'AIAP', [
            'nonce' => wp_create_nonce('aiap_ajax_nonce')
        ]);
    }


    function ajax_run_start() {
        check_ajax_referer('aiap_ajax_nonce');
        
        $job_id = wp_generate_uuid4();
        set_transient('aiap_job_'.$job_id, [
            'step' => 'topic',
            'message' => '題材生成中...',
            'done' => false
        ...
        wp_remote_post(admin_url('admin-ajax.php'), [
            ...
            'body' => [
                'action' => 'aiap_do_job',
                'job_id' => $job_id,
                '_wpnonce' => wp_create_nonce('aiap_job_nonce')
            ]
        ]);
    ...
    function ajax_check_progress() {
        check_ajax_referer('aiap_ajax_nonce');
        
        $job_id = sanitize_text_field($_POST['job_id']);
        $status = get_transient('aiap_job_'.$job_id);
    ...
    function ajax_do_job() {
        check_ajax_referer('aiap_job_nonce');
        ...
        try {
            ...
            // 題材生成
            $this->update_job_status($job_id, 'topic', '題材生成中...');
            list($title, $angle, $outline) = $this->content_generator->generate_topic($api_key);
            
            // 本文生成
            $this->update_job_status($job_id, 'content', '本文生成中...');
            ...
            // 投稿作成
            $this->update_job_status($job_id, 'post', '投稿作成中...');
            ...
            if (!empty($o['gen_featured'])) {
                $this->update_job_status($job_id, 'image', 'アイキャッチ画像生成中...');
            ...
            // 完了
            $this->update_job_status($job_id, 'done', '記事生成が完了しました', true, get_edit_post_link($post_id, ''));

        } catch (Exception $e) {
            $this->update_job_status($job_id, 'error', 'エラーが発生しました: ' . $e->getMessage(), true);
        }

        wp_die();
    }

    private function update_job_status($job_id, $step, $message, $done = false, $edit_url = '') {
        set_transient('aiap_job_'.$job_id, [
            'step' => $step,
            'message' => $message,
            'done' => $done,
