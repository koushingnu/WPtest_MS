...
EOT;
            ...
            if(!empty($o['gen_featured'])){
                try {
                    ...
                    if ($thumbnail_id) {
                        error_log('AI Auto Poster: 既存のアイキャッチ画像を削除 ID=' . $thumbnail_id);
                        delete_post_thumbnail($post_id);
                    }

                    $result = set_post_thumbnail($post_id, $att_id);
                    if (!$result) {
                        error_log('AI Auto Poster アイキャッチ設定失敗: post_id=' . $post_id . ', att_id=' . $att_id);
                        // 手動でポストメタを設定
                        update_post_meta($post_id, '_thumbnail_id', $att_id);
                        error_log('AI Auto Poster: 手動でメタデータを設定 _thumbnail_id=' . $att_id);
                    } else {
                        error_log('AI Auto Poster: アイキャッチ画像設定成功');
                    }

                    // 設定の確認
                    $check_id = get_post_thumbnail_id($post_id);
                    if ($check_id != $att_id) {
                        error_log('AI Auto Poster アイキャッチ設定の不一致: expected=' . $att_id . ', actual=' . $check_id);
                        throw new Exception('アイキャッチの設定確認に失敗');
                    }
    ...
    private function save_base64_image_as_attachment($b64, $title, $post_id, &$msg='') {
        error_log('AI Auto Poster 画像保存開始: title=' . $title . ', post_id=' . $post_id);

        // アップロードディレクトリの確認
        $upload = wp_upload_dir();
        if (!empty($upload['error'])) {
            $msg = 'アップロードディレクトリエラー: ' . $upload['error'];
            error_log('AI Auto Poster ' . $msg);
            return 0;
        }
        ...
        // ファイル名とパスの設定
        $fname = sanitize_title($title) . '-' . time() . '.' . $ext;
        $filepath = trailingslashit($upload['path']) . $fname;
        $fileurl = trailingslashit($upload['url']) . $fname;

                    $this->add_debug_log('💾 保存パス: ' . $filepath);
        ...
        if (file_put_contents($filepath, $data) === false) {
            ...
            return 0;
                    ...
                    $this->add_debug_log('✅ ファイル保存完了: ' . $fname);
        ...
        // 添付ファイルのメタデータを生成
        $attach_data = wp_generate_attachment_metadata($att_id, $filepath);
