...
class AIAP_Lite_Box {
    ...
    function register(){
        ...
        add_settings_field('gen_featured','アイキャッチ自動生成（gpt-image-1）', function(){
            ...
            echo '<p class="description">※任意。雰囲気・色味・抽象度のヒント。人物の顔アップなど具体個人は避けてください。</p>';
        }, 'aiap-lite','main');
    }
    function page(){ ?>
        <div class="wrap">
...
EOT;
        ...
        try{
            ...
            if (!empty($o['gen_featured'])) {
                ...
                try{
                    ...
                    $b64 = $this->openai_image($api_key, $img_prompt, '1792x1024');
                    if ($b64) {
                        $att_id = $this->save_base64_image_as_attachment($b64, $final_title, $post_id, $thumb_msg);
                        if ($att_id) {
                            if (!set_post_thumbnail($post_id, $att_id)) {
                                $thumb_msg = 'アイキャッチの設定に失敗しました（set_post_thumbnail）';
                            }
                        } else {
                            if (!$thumb_msg) $thumb_msg = '画像の保存に失敗しました';
                        }
                    } else {
                        $thumb_msg = '画像生成APIの応答が不正/失敗';
                    }
                } catch (\Throwable $ie) {
                    $thumb_msg = '画像生成処理でエラー: '.$ie->getMessage();
                }
                if ($thumb_msg) {
            ...
            $this->set_notice('error','記事生成に失敗: '.esc_html($e->get_message()));
            wp_safe_redirect(admin_url('options-general.php?page=aiap-lite')); exit;
        }
    }
    ...
    private function openai_image($api_key, $prompt, $size='1024x1024'){
    ...
    private function build_image_prompt($title, $angle, $hint=''){
        $pieces = array_filter([
            ...
            "タイトル: {$title}",
            "切り口: {$angle}",
            "内容を象徴する抽象的・メディカル系のビジュアル（頭皮/毛髪イメージ、分子・グラフ、清潔感のある背景）。",
            "文字は入れない。ロゴ不要。UI部品なし。高級感・清潔感・信頼感。",
            $hint ? "雰囲気のヒント: {$hint}" : ""
        ]);
