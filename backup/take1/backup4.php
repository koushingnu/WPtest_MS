 ...
 * Description: 最小構成。OpenAIのJSONをGutenbergブロックへ変換（見出し/段落/リスト/ボックス対応）+ タイトル/題材のバリエーション + 実行後は完了ログ表示 + アイキャッチ自動生成(gpt-image-1)
 * Version: 0.3.3
 * Author: You
 */
...
class AIAP_Lite_Box {
    ...
    function register(){
        ...
        add_settings_field('gen_featured','アイキャッチ自動生成（gpt-image-1）', function(){
            $o=get_option(self::OPT_KEY,[]);
            ...
            echo '<p class="description">※任意。雰囲気・色味・抽象度のヒント。人物の特定回避がおすすめ。</p>';
        }, 'aiap-lite','main');
    }
    function page(){ ?>
        <div class="wrap">
    ...
    function handle_run(){
        ...
        $prompt1 = <<<EOT
根本テーマは常に「AGA」。ただし毎回、焦点（切り口）を変え、前回と同じ趣旨を繰り返さない題材を1つだけ。
例）生活習慣/睡眠/食事/ストレス/運動/シャンプー/季節/年代/女性の薄毛と鑑別/自己流ケアの落とし穴/医療と市販の役割/通院の続け方 など。

制約:
- 出力はJSONのみ。
- "title" は32字以内・句点なし・本文は必ずこのタイトルに沿う。
- "angle" は切り口を短く。
- "outline" は本文のH2候補（4〜6個）。
- 固定テンプレ（同一表現の繰り返し）を避ける。
- ランダム種: {$seed}
...
EOT;
        ...
        $prompt2 = <<<EOT
ヘアクリニック向けブログ。必ずタイトルに沿い、angleとoutlineを反映。
- title: {$title}
- angle: {$angle}
- outline(H2候補): {$outline_json}
分量: 1600〜2300字（日本語）。断定しすぎない表現も適宜使用。
...
ルール:
- emphasisは {"text","style"}。styleは "strong" | "underline_yellow" | "underline_pink"。
- 箇条書き items はテキストのみ。先頭に「・」は書かない（レンダリング側で付与）。
- 同じ表現の使い回しを避け、{$seed} を参考に言い換える。
EOT;

        try{
            $data = $this->openai_json($api_key, $system2, $prompt2, 2200, 0.65);
            if (!is_array($data) || empty($data['sections'])) throw new Exception('JSON解析失敗 or sections無し');
            ...
            if (!empty($o['gen_featured'])) {
                ...
                try{
                    ...
                    // まず 1024x1024 で生成（安定サイズ）。失敗時に安全プロンプトへ差し替えて再試行。
                    $b64 = $this->openai_image($api_key, $img_prompt, '1024x1024', $api_err1);
                    if (!$b64) {
                        $safe_prompt = $this->build_image_prompt('AGAの抽象的な医療イメージ', '安全な抽象表現', $hint);
                        $b64 = $this->openai_image($api_key, $safe_prompt, '1024x1024', $api_err2);
                    }
                    if ($b64) {
                        $att_id = $this->save_base64_image_as_attachment($b64, $final_title, $post_id, $thumb_msg);
                        if ($att_id) {
                            if (!set_post_thumbnail($post_id, $att_id)) {
                                $thumb_msg = 'set_post_thumbnail に失敗しました。';
                            }
                        } else {
                            if (!$thumb_msg) $thumb_msg = '画像の保存に失敗しました。';
                        }
                    } else {
                        $thumb_msg = '画像生成APIエラー: '.esc_html($api_err2 ?: $api_err1 ?: '不明');
                    }
                } catch (\Throwable $ie) {
                    $thumb_msg = '画像生成処理エラー: '.$ie->getMessage();
                }
                if ($thumb_msg) {
                    $this->set_notice('error','記事は作成、ただしアイキャッチ未設定: '.esc_html($thumb_msg), get_edit_post_link($post_id,''));
                    wp_safe_redirect(admin_url('options-general.php?page=aiap-lite')); exit;
                }
            }
     ...
     * - 安定サイズ 1024x1024
     * - 失敗時の詳細メッセージを $err に返す
     * - 成功時は base64 を返す
     */
    private function openai_image($api_key, $prompt, $size='1024x1024', &$err=''){
    $err = '';
    $resp = wp_remote_post('https://api.openai.com/v1/images/generations', [
        ...
        'body' => wp_json_encode([
            'model'   => 'dall-e-3',
            'prompt'  => $prompt,
            'size'    => $size,      // 例: 1024x1024
            'n'       => 1,
            // 'quality' => 'medium',
            // ※ ここで 'response_format' は送らない（エラーの原因）
        ]),
    ]);
    ...
    if ($code !== 200) {
        ...
        $err = $j['error']['message'] ?? ('HTTP '.$code.' / '.mb_substr($raw,0,300));
        return null;
    }

    $body = json_decode($raw, true);
    if (!isset($body['data'][0])) { $err = '画像データが空です'; return null; }

    // 1) URL があればダウンロードして base64 に
    if (!empty($body['data'][0]['url'])) {
        $bin = $this->download_image_binary($body['data'][0]['url'], $dlerr);
        if ($bin !== null) return base64_encode($bin);
        $err = $dlerr ?: '画像URLの取得に失敗';
        return null;
    }

    // 2) URLが無い場合は b64_json を見る（返ってくる場合もある）
    if (!empty($body['data'][0]['b64_json'])) {
        return $body['data'][0]['b64_json'];
    }

    $err = 'API応答に url / b64_json が含まれていません';
    return null;
}
    ...
    private function save_base64_image_as_attachment($b64, $title, $post_id, &$msg=''){
        ...
        if (!empty($upload['error'])) { $msg='アップロードディレクトリエラー: '.$upload['error']; return 0; }
        if (!wp_mkdir_p($upload['path'])) { $msg='アップロード先の作成に失敗: '.$upload['path']; return 0; }

        $data = base64_decode($b64);
        if (!$data) { $msg='base64デコードに失敗'; return 0; }
        ...
        $base = sanitize_title($title ?: 'featured').'-'.time().'.'.$ext;
        $filename = wp_unique_filename($upload['path'], $base);
        $filepath = trailingslashit($upload['path']).$filename;

        if (file_put_contents($filepath, $data) === false) { $msg='画像ファイルの書き込みに失敗'; return 0; }

        $fileurl  = trailingslashit($upload['url']).$filename;
        $attachment = [
            'post_mime_type' => $mime,
            'post_title'     => sanitize_text_field($title ?: 'featured'),
            'post_content'   => '',
            'post_status'    => 'inherit',
            'guid'           => $fileurl,
        ];
        $attach_id = wp_insert_attachment($attachment, $filepath, $post_id);
        if (is_wp_error($attach_id) || !$attach_id) { $msg='添付登録に失敗'; return 0; }
    ...
    $err = '';
    $r = wp_remote_get($url, ['timeout' => 300]);
